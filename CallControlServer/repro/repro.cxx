...
main(int argc, char** argv)
{
	...
	//如果请求目标和proxy不处于同一个域,则进行AmIResponsible认证,如果来自非受信站点且请求者和proxy不属于同一个域则会失败
	//不过此次会产生一个漏洞,那就是其他域的请求者都不经过认证就可以通过Proxy访问本域的设备了.
	//不过也正因为这个漏洞使得跨接入的访问成为可能.
    ...
    //zhangjun change begin: 支持所需方法和content type
...
//   if (!args.mNoRegistrar || args.mCertServer) //zhangjun comment
//   {                                           //zhangjun comment 
    dum = new DialogUsageManager(stack);
//      dum->setMasterProfile(profile);          //zhangjun comment
...
//   }                                           //zhangjun comment
	...
	methodList.push_back(resip::SUBSCRIBE);
        //profile->addSupportedMimeType( resip::SUBSCRIBE, resip::Mime("application", "pkix-cert") );
	profile->addSupportedMimeType( resip::SUBSCRIBE, resip::Mime("application", "global_eye_v10+xml") );
	...
	if (!args.mNoChallenge)
	{
	    //!!!如何能只认证注册请求呢? 只能是修改ServerAuthManager的process函数了
