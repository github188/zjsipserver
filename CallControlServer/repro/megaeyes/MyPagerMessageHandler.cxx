...
void
MyServerPagerMessageHandler::onMessageArrived( resip::ServerPagerMessageHandle handle, const resip::SipMessage& message )
{
    ...
    {
	MyServerSubscriptionHandler *handler = dynamic_cast<MyServerSubscriptionHandler*>( mDum.getServerSubscriptionHandler("alarm") );
	if ( handler )
	{
...
int 
MyServerPagerMessageHandler::parseMessage( const resip::Data &xmlBody, std::string &msgType, std::string &msgSrcId )
{
    CMarkupSTL SubXML;
    if ( xmlBody.size() > 0 )
    {
